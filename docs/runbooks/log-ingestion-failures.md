...
4. If the file sink exists but Loki queries stay empty, inspect the collector configuration mounted at `ops/monitoring/otel-lgtm/otelcol-config.yaml` and verify the app log file is readable inside the `observability` container.
