...
- Immutable registry routes are implemented in [app/interface/api/skills.py](../../app/interface/api/skills.py):
  ...
  - `GET /skills/{id}`
- Publish, exact fetch, duplicate rejection, checksum generation, and read-time integrity verification are handled by [app/core/skills/registry.py](../../app/core/skills/registry.py).
...
flowchart LR
    ...
    API --> Core["SkillRegistryService<br/>app/core/skills/registry.py"]
...
Why this shape:
- The service owns the transaction boundary between contract validation, artifact storage, metadata persistence, and audit emission. See [app/core/skills/registry.py](../../app/core/skills/registry.py).
...
- Duplicate protection is layered. The service pre-checks `(skill_id, version)`, the filesystem adapter guards immutable paths, and the database enforces a unique constraint on `(skill_fk, version)`. See [app/core/skills/registry.py](../../app/core/skills/registry.py), [app/persistence/artifact_store.py](https://github.com/y0ncha/Aptitude/blob/1511320d6002088dd240ca76413dda7fbe650703/app/persistence/artifact_store.py), and [alembic/versions/0002_immutable_skill_registry.py](https://github.com/y0ncha/Aptitude/blob/2af854594fcd3c2b75b7363557f368272a430b13/alembic/versions/0002_immutable_skill_registry.py).
- Exact version fetches always recompute `sha256` over stored artifact bytes before returning a response, so corruption is detected on the read path rather than assumed away. See [app/core/skills/registry.py](../../app/core/skills/registry.py) and [tests/integration/test_skill_registry_endpoints.py](../../tests/integration/test_skill_registry_endpoints.py).
